...
#define MEMALLOC_SIZE   (4096) //(SZ_4M)
#else
...
unsigned int size_table_0[] = {
         ...
         38, 38, 38,                                                            // For VT use case
         56, 60, //57, 57, 60, //57,               // HVGA 480*320*1.5
         124, 124, 124, 124,            // VGA 640*480*1.5
         150, 150, 150, 150                             // VGA 640*480*2
};
				...
				unsigned int phy_start_address)
{
	...
	for (i = 0; i < HLINA_NUMBER_OF_CHUNKS; i++) {
		...
		hlina_chunks[i].file_id = NULL;
#if defined (CONFIG_BMEM)
		hlina_chunks[i].size = 0; // 4096 * size_table_0[i];
#else
